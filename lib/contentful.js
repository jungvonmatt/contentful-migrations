...
const getLinkedEntries = (entries, allEntries, collectedIds = []) => {
  if ((entries || []).length === 0) {
    return [];
  }
  ...
  const newIds = linkedIds.filter((id) => !collectedIds.includes(id) && !entryIds.includes(id));
  const newEntries = allEntries.filter((entry) => newIds.includes(getContentId(entry)));

  return [...newEntries, ...getLinkedEntries(newEntries, allEntries, [...collectedIds, ...entryIds])];
};
...
const getContent = async (options, all) => {
  ...
  // GET CONTENT
  // const { entries, assets } = await contentfulExport({
  //   spaceId,
  //   managementToken,
  //   environmentId,
  //   includeDrafts: true,
  ...
  if (!all && contentType) {
    ...
    const linkedEntries = getLinkedEntries(baseEntries, entries);
    const transferEntries = [...baseEntries, ...linkedEntries];
    return { entries: transferEntries, assets: getLinkedAssets(transferEntries, assets), contentTypes };
  }
