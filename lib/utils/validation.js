 ...
 *   ...
 *
 *   await utils.addLinkContentTypeValues('contentTypeId', 'fieldId', ['value']);
 *   await utils.addInValues('contentTypeId', 'fieldId', ['value']);
 *   await utils.removeLinkContentTypeValues('contentTypeId', 'fieldId', ['value']);
 *   await utils.removeInValues('contentTypeId', 'fieldId', ['value']);
 *
 * });
 *
 */
 const getValidationHelper = (migration, context) => {
  const { makeRequest } = context;

  const addValidationValues = (validations = [], key, values = []) => validations.map((validation) => {
    if (validation?.[key]) {
      if (!Array.isArray(values)) {
        values = [values];
      }
      if (!Array.isArray(validation[key])) {
        throw new Error(`addValidationValues can only be used on arrays. validation.${key} is typeof ${typeof validation[key]}`);
      }
  ...
  const removeValidationValues = (validations = [], key, values = []) => validations.map((validation) => {
    if (validation?.[key]) {
      if (!Array.isArray(values)) {
        values = [values];
      }
      if (!Array.isArray(validation[key])) {
        throw new Error(`removeValidationValues can only be used on arrays. validation.${key} is typeof ${typeof validation[key]}`);
      }
  ...
  const modifyValidationValuesForType = async (validationKey, method, contentTypeId, fieldId, typeIds) => {
    ...
    if (type === TYPE_LINK) {
      const ct = migration.editContentType(contentTypeId);
      ct.editField(fieldId).validations( method(items?.validations ?? [], validationKey, typeIds));
    }
  };
