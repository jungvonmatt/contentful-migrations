...
const getConfig = async (args, required = []) => {
  const { configFile, ...overrides } = args;
  ...
  const config = {
    directory: path.resolve(path.dirname(result.filepath || ''), args.directory || 'migrations'),
    ...(result.config || {}),
  };

  const missingStorage = result.missing.includes('storage');

  return { ...config, missingStorageModel: missingStorage && config.storage === STORAGE_CONTENT };
};
