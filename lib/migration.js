...
const getClient = async (options) => {
  const { space, accessToken, environment, branch } = options || {};
  ...
  const contentfulSpace = await client.getSpace(space);
  const { items: environments } = await contentfulSpace.getEnvironments();

  const environmentIds = (environments || []).map(env.sys.id);

  const targetEnvironment = environment || branch;
  if (targetEnvironment && environmentIds.includes(targetEnvironment)) {
    return contentfulSpace.getEnvironment(targetEnvironment);
  }

  return contentfulSpace.getEnvironment(DEFAULT_ENVIRONMENT);
};
...
const createMigration = async (config) => {
  ...
  if (typeof version === 'undefined' && !fs.existsSync(initialFilename)) {
    ...
    await writeFileAsync(initialFilename, initialMigrationContent);
