...
const transferContent = async (config) => {
  ...
  const { entries: sourceEntries, assets: sourceAssetsBase } = await getContent({
    ...config,
  ...
  let assetOverwrites = {};
  if (diffConflicts) {
    assetOverwrites = await inquirer.prompt(assetPrompts);
  } else if (forceOverwrite) {
    assetOverwrites = assetPrompts.reduce((result, { name }) => ({ ...result, [name]: true }), {});
  } else {
    assetOverwrites = assetPrompts.reduce((result, { name }) => ({ ...result, [name]: false }), {});
  }

  const skipAssetIds = conflictAssetIds.filter((id) => {
    const { [id]: overwrite } = assetOverwrites;
    return !overwrite;
  });
