...
const transferContent = async (config) => {
  const { contentType, directory, sourceEnvironment, destEnvironment, spaceId, accessToken: managementToken } =
    config || {};
  ...
  const assetAnswers = await inquirer.prompt(assetPrompts);

  const skipAssetIds = conflictAssetIds.filter((id) => {
    const { [id]: overwrite } = assetAnswers;
    return !overwrite;
  });

  const assets = sourceAssets.filter((entry) => !skipAssetIds.includes(getContentId(entry)));

  const br = Object.keys(assetAnswers).length || Object.keys(entryAnswers).length ? '\n' : '';
  ...
  // console.log(
  //   'ENTRIES:',
  //   entries.map((entry) => `[${getContentTypeId(entry)}] ${getContentId(entry)}`)
  // );
  // console.log(
  //   'ASSETS:',
  //   assets.map((asset) => getContentId(asset))
  // );
