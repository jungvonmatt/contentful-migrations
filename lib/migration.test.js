...
const makeTempProject = async (
  packageJson = { name: "fixture-project", version: "1.0.0" },
) => {
...
describe("migration helpers", () => {
  ...
  beforeEach(() => {
    ...
    oxfmt = {
      format: vi.fn((file, content) => ({ code: content })),
    };
    runMigration = vi.fn();
    ...
    ({ createMigration, fetchMigration, executeMigration, runMigrations } =
      loadCjsModule("./lib/migration.js", {
        "fs-extra": fsExtra,
        oxfmt: oxfmt,
        "contentful-migration": { runMigration },
  ...
  it("passes filename and content to oxfmt format", async () => {
    const cwd = await makeTempProject();
    ...
    expect(oxfmt.format).toHaveBeenCalledWith(
      expect.stringContaining("100-migration.js"),
      expect.any(String),
  ...
  it("logs and rethrows migration discovery errors", async () => {
    ...
    await expect(
      runMigrations({ managementToken: "token", spaceId: "space-1" }),
    ).rejects.toBe(error);
