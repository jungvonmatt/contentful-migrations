...
const diff = (source, dest, contentTypes) => {
  ...
  const fieldData = changedFields.map((fieldId) => {
    ...
    if (type === TYPE_LINK) {
      ...
      return `${chalk.bold(name)} (${linkType}): ${chalk.reset.green(sourceValueId || 'empty')}  ${chalk.reset.red(
        destValueId || 'deleted'
      )}`;
    }
      ...
      const array = Array(Math.max(sourceArray.length, destArray.length))
        ...
        .map((_, index) => {
          if (sourceArray[index] === destArray[index]) {
            return chalk.grey(sourceArray[index]);
          }

          return `${chalk.reset.green(sourceArray[index] || '')} ${chalk.reset.red(destArray[index] || '')}`;
        });
